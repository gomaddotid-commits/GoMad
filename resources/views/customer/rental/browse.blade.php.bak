...
<div class="container-magazine py-8" x-data="{ filterOpen: false }">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-8">
        <div>
            <h1 class="text-2xl md:text-3xl font-bold text-[#111827] mb-2">Cari Mobil Rental</h1>
            <p class="text-gray-500 font-light">Sewa mobil lepas kunci atau dengan supir. Booking mudah, harga transparan.</p>
        </div>
        
        <div class="flex items-center gap-3">
            @if($hasFilter)
            <a href="{{ route('customer.rental.browse') }}" class="text-xs text-[#BA1826] hover:underline font-medium whitespace-nowrap">Reset Filter</a>
            @endif
            <button @click="filterOpen = !filterOpen" 
                    class="lg:hidden flex items-center gap-2 px-4 py-2 border border-[#E5E7EB] rounded-[12px] text-sm font-medium hover:bg-[#F9FAFB] transition">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"/></svg>
                Filter
                @if($hasFilter)
                <span class="w-2 h-2 bg-[#BA1826] rounded-full"></span>
                @endif
            </button>
        </div>
    </div>
    ...
    <div class="grid lg:grid-cols-4 gap-8">
        {{-- SIDEBAR FILTER --}}
        {{-- Logika: Di mobile (hidden), saat tombol ditekan (block). Di desktop (lg:block dan lg:col-span-1) --}}
        <div class="lg:col-span-1" :class="filterOpen ? 'block' : 'hidden'">
            <div class="lg:block card-gomad p-5 sticky top-24 border-[#E5E7EB]">
                ...
                <form action="{{ route('customer.rental.browse') }}" method="GET" class="space-y-4">
                    <div>
                        ...
                        <select name="type" class="w-full px-0 py-2 border-b-2 border-[#E5E7EB] focus:border-[#BA1826] outline-none bg-transparent font-medium text-[#111827] text-sm appearance-none cursor-pointer">
                    ...
                    <div x-data="locationFilter()">
                        <div>
                            ...
                            <select name="province_code" x-model="province" @change="loadCities()" 
                                    class="w-full px-0 py-2 border-b-2 border-[#E5E7EB] focus:border-[#BA1826] outline-none bg-transparent font-medium text-[#111827] text-sm appearance-none cursor-pointer">
                    ...
                    <div>
                        ...
                        <input type="date" name="date" value="{{ request('date') }}" 
                               class="w-full px-0 py-2 border-b-2 border-[#E5E7EB] focus:border-[#BA1826] outline-none bg-transparent font-medium text-[#111827] cursor-pointer">
                    </div>

                    <div>
                        <label class="block text-xs font-mono uppercase tracking-wider text-gray-500 mb-1">Agency</label>
                        <select name="agency_id" class="w-full px-0 py-2 border-b-2 border-[#E5E7EB] focus:border-[#BA1826] outline-none bg-transparent font-medium text-[#111827] text-sm appearance-none cursor-pointer">
                    ...
                    <div>
                        ...
                        <select name="transmission" class="w-full px-0 py-2 border-b-2 border-[#E5E7EB] focus:border-[#BA1826] outline-none bg-transparent font-medium text-[#111827] text-sm appearance-none cursor-pointer">
                    ...
                    <div>
                        ...
                        <div class="flex gap-2 items-center">
                            <input type="number" name="price_min" value="{{ request('price_min') }}" 
                                   class="w-full px-0 py-2 border-b-2 border-[#E5E7EB] focus:border-[#BA1826] outline-none bg-transparent font-medium text-[#111827] text-sm"
        ...
        <div class="lg:col-span-3">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6 border-b border-[#E5E7EB] pb-4">
                <p class="text-sm text-gray-500 font-light">
                    Menampilkan <strong class="text-[#111827]">{{ $vehicles->total() }}</strong> mobil
                    @if(request('type'))
            ...
            <div class="card-gomad p-12 text-center border-[#E5E7EB]">
                ...
                <a href="{{ route('customer.rental.browse') }}" class="inline-block mt-4 text-[#BA1826] hover:underline font-medium">Reset Filter</a>
            </div>
            @else
            <div class="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
                ...
                <div class="bg-white border border-[#E5E7EB] rounded-[12px] overflow-hidden shadow-gomad hover:border-[#BA1826] transition-colors group">
                    <div class="h-40 bg-[#F9FAFB] flex items-center justify-center overflow-hidden border-b border-[#E5E7EB] relative">
                    ...
                    <div class="p-4">
                        <div class="flex items-center gap-2 mb-2">
                            <div class="w-8 h-8 rounded-full bg-[#F9FAFB] flex items-center justify-center overflow-hidden flex-shrink-0 border border-[#E5E7EB]">
                                @if($agency->logo)<img src="{{ $agency->logo }}" class="w-full h-full object-cover">@else<span class="text-sm">🏢</span>@endif
                            </div>
                            <div class="min-w-0">
                                <p class="font-semibold text-xs text-[#111827] truncate">{{ $agency->agency_name }}</p>
                        ...
                        <div class="flex flex-wrap gap-1 mt-1.5">
                            ...
                            <span class="px-1.5 py-0.5 bg-[#F9FAFB] text-[9px] font-mono uppercase rounded text-gray-500">{{ $specs['transmission'] == 'automatic' ? 'AT' : 'MT' }}</span>
                            @endif
                            @if($specs['ac'] ?? true)
                            <span class="px-1.5 py-0.5 bg-[#F9FAFB] text-[9px] font-mono uppercase rounded text-gray-500">AC</span>
                            @endif
                            <span class="px-1.5 py-0.5 bg-[#F9FAFB] text-[9px] font-mono uppercase rounded text-gray-500">{{ $vehicle->capacity }} Seat</span>
                        </div>

                        <div class="mt-3 bg-[#F9FAFB] border border-[#E5E7EB] rounded-[12px] p-2.5">
                        ...
                        <div class="mt-2">
                            <div class="flex items-center justify-between mb-1">
                                ...
                                <button type="button" onclick="openCalendarModal('{{ $vehicle->id }}')" class="text-[9px] text-[#BA1826] hover:underline font-medium">📅 Cek Tanggal</button>
                            </div>
                        ...
                        <a href="{{ route('customer.rental.create', $setting) }}" 
                           class="block w-full text-center bg-[#BA1826] text-white py-2 rounded-[10px] font-semibold hover:bg-[#8A0F18] transition text-xs mt-3">
                            Sewa Sekarang
                        </a>
                    </div>
                </div>
