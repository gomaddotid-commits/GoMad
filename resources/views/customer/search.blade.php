...
<div class="container-magazine py-8" x-data="{ filterOpen: false, searchMode: '{{ request('origin_city_code') || request('destination_city_code') ? 'city' : 'text' }}' }">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-8">
        <div>
            <h1 class="text-2xl md:text-3xl font-bold text-[#111827] mb-2">Cari Jadwal Travel</h1>
            <p class="text-gray-500 font-light">Temukan jadwal yang sesuai dengan kebutuhan Anda.</p>
        </div>
        
        <div class="flex items-center gap-3">
            @if($hasFilter)
            <a href="{{ route('customer.search') }}" class="text-xs text-[#BA1826] hover:underline font-medium whitespace-nowrap">Reset Filter</a>
            @endif
    ...
    <div class="grid lg:grid-cols-4 gap-8">
        {{-- SIDEBAR FILTER --}}
        {{-- Logika: Di mobile (hidden), tapi saat tombol ditekan (block). Di desktop (lg:block dan lg:col-span-1) --}}
            ...
            <div class="lg:block card-gomad p-5 sticky top-24 border-[#E5E7EB]">
                ...
                <form action="{{ route('customer.search') }}" method="GET" class="space-y-4">
                    ...
                    <div x-show="searchMode === 'city'" x-cloak>
                        <div>
                            <label class="block text-xs font-mono uppercase tracking-wider text-gray-500 mb-1">Kota Asal</label>
                            <select name="origin_city_code" class="w-full px-0 py-2 border-b-2 border-[#E5E7EB] focus:border-[#BA1826] outline-none bg-transparent text-[#111827] text-sm transition">
                        ...
                        <div class="mt-4">
                            <label class="block text-xs font-mono uppercase tracking-wider text-gray-500 mb-1">Kota Tujuan</label>
                            <select name="destination_city_code" class="w-full px-0 py-2 border-b-2 border-[#E5E7EB] focus:border-[#BA1826] outline-none bg-transparent text-[#111827] text-sm transition">
                                <option value="">Semua Kota</option>
                                @foreach($allCities as $city)
        ...
        <div class="lg:col-span-3">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex bg-[#F9FAFB] rounded-lg p-1">
                        <a href="{{ request()->fullUrlWithQuery(['view' => 'grid']) }}" 
                           class="px-3 py-1.5 rounded-md text-sm transition {{ $viewMode == 'grid' ? 'bg-white shadow text-[#BA1826] font-medium' : 'text-gray-500 hover:text-[#111827]' }}">
                            <svg class="w-4 h-4 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
                        </a>
                        <a href="{{ request()->fullUrlWithQuery(['view' => 'list']) }}" 
                           class="px-3 py-1.5 rounded-md text-sm transition {{ $viewMode == 'list' ? 'bg-white shadow text-[#BA1826] font-medium' : 'text-gray-500 hover:text-[#111827]' }}">
            ...
            <div class="card-gomad p-12 text-center border-[#E5E7EB]">
                ...
                <a href="{{ route('customer.search') }}" class="inline-block mt-4 text-[#BA1826] hover:underline font-medium">Reset Filter</a>
            </div>
            @else
                @if($viewMode == 'grid')
                <div class="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
                    @foreach($schedules as $s)
                    <div class="bg-white border border-[#E5E7EB] rounded-[12px] p-4 shadow-gomad hover:border-[#BA1826] transition-colors group">
                        <div class="flex items-center gap-3 mb-3">
                            ...
                            <div class="min-w-0">
                                <p class="font-semibold text-sm text-[#111827] truncate">{{ $s->agency->agency_name }}</p>
                        ...
                        <p class="text-sm font-medium text-[#111827] mb-1">{{ $s->route->route_name }}</p>
                        <p class="text-xs text-gray-500 font-light mb-3">
                            {{ $s->route->origin_city_name }} → {{ $s->route->destination_city_name }}
                        </p>

                        <div class="bg-[#F9FAFB] border border-[#E5E7EB] rounded-[12px] p-3 mb-3">
                            <div class="flex justify-between text-sm">
                                <span class="font-medium text-[#111827]">{{ $s->departure_date->format('d M Y') }}</span>
                            ...
                            <div class="flex justify-between text-xs text-gray-500 mt-1 font-mono uppercase tracking-wider">
                                ...
                                <span class="text-[#BA1826]">{{ $s->travel_class }}</span>
                            </div>
                        </div>

                        <div class="flex justify-between items-center border-t border-[#E5E7EB] pt-3">
                            <div>
                                <p class="font-bold text-[#BA1826] font-mono">Rp {{ number_format($s->price_per_seat, 0, ',', '.') }}</p>
                                <p class="text-xs {{ $s->available_seats > 0 ? 'text-green-600' : 'text-[#BA1826]' }} font-mono uppercase tracking-wider">
                                    {{ $s->available_seats > 0 ? $s->available_seats . ' kursi' : 'Penuh' }}
                                </p>
                            </div>
                            @if($s->available_seats > 0)
                            <a href="{{ route('customer.booking.create', $s) }}" class="btn-gomad-primary text-sm py-2 px-4 rounded-[10px]">Booking</a>
                            @endif
                        </div>
                    </div>
                    @endforeach
                ...
                <div class="space-y-3">
                    @foreach($schedules as $s)
                    <div class="bg-white border border-[#E5E7EB] rounded-[12px] p-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 shadow-gomad hover:border-[#BA1826] transition-colors">
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 rounded-full bg-[#F9FAFB] flex items-center justify-center overflow-hidden flex-shrink-0 border border-[#E5E7EB]">
                                @if($s->agency->logo)<img src="{{ $s->agency->logo }}" class="w-full h-full object-cover">@else<span class="text-xl">🏢</span>@endif
                            </div>
                            <div>
                                <div class="flex items-center gap-2">
                                    <p class="font-bold text-[#111827]">{{ $s->agency->agency_name }}</p>
                                ...
                                <p class="text-xs text-gray-400 font-mono">
                                    ...
                                    <span class="text-[#BA1826] uppercase">{{ $s->travel_class }}</span>
                                </p>
                            </div>
                        </div>
                        <div class="flex items-center gap-4 flex-shrink-0">
                            <div class="text-right">
                                <p class="font-bold text-[#BA1826] font-mono text-lg">Rp {{ number_format($s->price_per_seat, 0, ',', '.') }}</p>
                                <p class="text-xs {{ $s->available_seats > 0 ? 'text-green-600' : 'text-[#BA1826]' }} font-mono uppercase tracking-wider">
                                    {{ $s->available_seats > 0 ? $s->available_seats . ' kursi' : 'Penuh' }}
                                </p>
                            </div>
                            @if($s->available_seats > 0)
                            <a href="{{ route('customer.booking.create', $s) }}" class="btn-gomad-primary text-sm py-2 px-5 rounded-[10px] whitespace-nowrap">Booking</a>
                            @endif
                        </div>
                    </div>
                    @endforeach
