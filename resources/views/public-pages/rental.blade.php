...
<div class="section">
    <div class="container-magazine">
        <div class="mb-8">
            <h1 class="text-3xl md:text-4xl font-bold text-[#111111] mb-2">Cari Rental Mobil</h1>
            <p class="text-gray-500 font-light">Sewa mobil untuk perjalanan Anda. Tersedia lepas kunci atau dengan supir.</p>
        </div>

        {{-- Filter --}}
        <div class="bg-white border border-[#E5E5E5] rounded-[12px] p-5 mb-8 shadow-sm">
            <form action="{{ route('rental.public') }}" method="GET" class="grid grid-cols-2 md:grid-cols-5 gap-4 items-end">
                <div>
                    ...
                    <select name="type" class="w-full px-0 py-2 border-b-2 border-[#E5E5E5] focus:border-[#C1121F] outline-none bg-transparent text-[#111111] text-sm">
                ...
                <div>
                    ...
                    <select name="agency_id" class="w-full px-0 py-2 border-b-2 border-[#E5E5E5] focus:border-[#C1121F] outline-none bg-transparent text-[#111111] text-sm">
                ...
                <div class="col-span-2 md:col-span-2 flex gap-3">
                    <button type="submit" class="flex-1 btn-gomad-primary py-2.5 text-sm rounded-[12px]">Cari</button>
                    @if(request()->anyFilled(['type', 'date', 'agency_id']))
                    <a href="{{ route('rental.public') }}" class="flex-1 border border-[#E5E5E5] text-gray-600 py-2.5 text-sm rounded-[12px] text-center hover:bg-[#F5F5F5] transition">Reset</a>
                    @endif
                </div>
            </form>
        </div>
        ...
        {{-- Results --}}
        <div class="flex justify-between items-center mb-6 border-b border-[#E5E5E5] pb-4">
            <p class="text-sm text-gray-500 font-light">
                Menampilkan <strong class="text-[#111111]">{{ $vehicles->total() }}</strong> kendaraan
            </p>
        </div>
        ...
        <div class="card-gomad p-12 text-center border-[#E5E5E5]">
            ...
            <a href="{{ route('rental.public') }}" class="inline-block mt-4 text-[#C1121F] hover:underline font-medium">Reset Filter</a>
        </div>
        @else
            ...
            <div class="bg-white border border-[#E5E5E5] rounded-[12px] overflow-hidden shadow-sm hover:border-[#C1121F] transition-colors group">
                ...
                <div class="p-5">
                    ...
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <div class="min-w-0">
                            <p class="font-semibold text-sm text-[#111111] truncate">{{ $agency->agency_name }}</p>
                    ...
                    {{-- Info Kendaraan --}}
                    <p class="font-bold text-[#111111]">{{ $vehicle->brand }} {{ $vehicle->model }} ({{ $vehicle->year }})</p>
                    ...
                    <div class="flex flex-wrap gap-1 mt-2">
                        ...
                        @foreach($specs as $key => $value)
                            ...
                            <span class="px-1.5 py-0.5 bg-[#F5F5F5] text-[10px] font-mono uppercase tracking-wider rounded text-gray-500">
                    ...
                    <div class="mt-3">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <button type="button" 
                                    ...
                                    class="text-[10px] text-[#C1121F] hover:underline font-medium">
                                📅 Cek Tanggal
                            </button>
                        </div>
                    ...
                    <div class="mt-4 border-t border-[#E5E5E5] pt-4">
                        ...
                        <a href="{{ route('customer.rental.create', $setting) }}" 
                           class="block w-full text-center bg-[#C1121F] text-white py-2.5 rounded-[12px] font-semibold hover:bg-[#8A0F18] transition text-sm">
                            Sewa Sekarang
                        </a>
                        @else
                        <a href="{{ route('login') }}" 
                           class="block w-full text-center border border-[#C1121F] text-[#C1121F] py-2.5 rounded-[12px] font-semibold hover:bg-[#C1121F] hover:text-white transition text-sm">
                            Login untuk Sewa
                        </a>
                        @endauth
                    </div>
...
<div id="calendarModal" class="fixed inset-0 bg-[#111111]/50 z-50 hidden items-center justify-center p-4" style="display:none;">
    <div class="bg-white rounded-[12px] shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto border border-[#E5E5E5]">
        ...
        <div class="sticky bottom-0 bg-white border-t border-[#E5E5E5] p-4 rounded-b-[12px] flex gap-3">
            ...
            @else
            <a href="{{ route('login') }}" class="flex-1 bg-[#C1121F] text-white py-2.5 rounded-[12px] font-semibold hover:bg-[#8A0F18] transition text-sm text-center">
                Login untuk Sewa
            </a>
            @endauth
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('[class*="mini-calendar-"]').forEach(function(container) {
        ...
        for (var i = 0; i < 30; i++) {
            ...
            var isBooked = bookedData[dateStr] !== undefined;
            var isToday = i === 0;
            
            var bgClass = 'bg-green-100 border border-green-300';
            var tooltip = dateStr + ' - Tersedia';
            if (isBooked) {
                bgClass = 'bg-red-200 border border-red-400';
                tooltip = dateStr + ' - Dibooking';
            }
            if (isToday) {
                bgClass = 'bg-yellow-200 border border-yellow-400 font-bold';
                tooltip = dateStr + ' - Hari Ini';
            }
            
            html += '<div class="w-3.5 h-3.5 rounded-sm ' + bgClass + '" title="' + tooltip + '"></div>';
        }
...
var modalBookedData = {};
var modalCurrentMonth = new Date().getMonth();
var modalCurrentYear = new Date().getFullYear();
var allVehicleData = {};

// Simpan data kendaraan untuk modal
document.querySelectorAll('[class*="mini-calendar-"]').forEach(function(el) {
    ...
    if (card) {
        var nameEl = card.querySelector('.font-bold.text-\\[\\#111111\\]');
...
function renderModalCalendar() {
    ...
    for (var day = 1; day <= daysInMonth; day++) {
        var dateStr = modalCurrentYear + '-' + 
                     String(modalCurrentMonth + 1).padStart(2, '0') + '-' + 
        ...
        if (isBooked) {
            ...
            var rentals = modalBookedData[dateStr];
            var rentalInfo = Array.isArray(rentals) ? rentals.map(function(r) { return r.rental_code + ' (' + r.type + ')'; }).join('\n') : 'Dibooking';
            tooltip = dateStr + ' - ' + rentalInfo;
        }
        if (isToday) {
            bgClass = 'bg-yellow-100 border border-yellow-300 font-bold';
            tooltip = dateStr + ' - Hari Ini';
        }
        if (isPast && !isToday) {
            bgClass = 'bg-gray-100 text-gray-300 border border-gray-200 cursor-not-allowed';
            tooltip = dateStr + ' - Sudah lewat';
        }
        
        html += '<div class="py-2 rounded-lg text-sm ' + bgClass + '" title="' + tooltip + '">';
        html += day;
        if (isBooked) {
            html += '<div class="text-[8px] text-red-500 leading-none">📌</div>';
        }
        html += '</div>';
    }
