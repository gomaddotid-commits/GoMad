...
<div class="section" x-data="{ loading: true }" x-init="setTimeout(() => loading = false, 600)">
    <div class="container-magazine">
        ...
        <div x-show="loading" x-cloak>
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                <x-skeleton-card type="vehicle" :count="12" />
            </div>
        </div>
                    ...
                    <div class="p-5">
                        ...
                        <div class="mt-3">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <button type="button" onclick="openCalendarModal('{{ $vehicle->id }}')" class="text-[10px] text-[#BA1826] hover:underline font-medium">📅 Cek Tanggal</button>
                            </div>
                        ...
                        <div class="mt-4 border-t border-[#E5E7EB] pt-4">
                            @auth
                            <a href="{{ route('customer.rental.create', $setting) }}" class="block w-full text-center bg-[#BA1826] text-white py-2.5 rounded-[10px] font-semibold hover:bg-[#8A0F18] transition text-sm">Sewa Sekarang</a>
                            @else
                            <a href="{{ route('login') }}" class="block w-full text-center border border-[#BA1826] text-[#BA1826] py-2.5 rounded-[10px] font-semibold hover:bg-[#BA1826] hover:text-white transition text-sm">Login untuk Sewa</a>
                            @endauth
                        </div>
                    </div>
                </div>
...
<div id="calendarModal" class="fixed inset-0 bg-[#111827]/50 z-50 hidden items-center justify-center p-4" style="display:none;">
    <div class="bg-white rounded-[12px] shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto border border-[#E5E7EB]">
        ...
        <div class="sticky bottom-0 bg-white border-t border-[#E5E7EB] p-4 rounded-b-[12px] flex gap-3">
            <button onclick="closeCalendarModal()" class="flex-1 border border-[#E5E7EB] py-2.5 rounded-[10px] font-medium hover:bg-[#F9FAFB] transition text-sm">Tutup</button>
            <a href="#" id="modalBookNow" class="flex-1 bg-[#BA1826] text-white py-2.5 rounded-[10px] font-semibold hover:bg-[#8A0F18] transition text-sm text-center">Sewa Sekarang</a>
        </div>
    </div>
</div>
...
document.addEventListener('click', function(e) { if (e.target.id === 'calendarModal') closeCalendarModal(); });
