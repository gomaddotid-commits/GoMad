    ...
    <div class="container-magazine">
        ...
        <div class="bg-white border border-[#E5E5E5] rounded-[12px] p-5 mb-8 shadow-sm">
            <form action="{{ route('rental.public') }}" method="GET" class="grid grid-cols-2 md:grid-cols-5 gap-4 items-end">
                ...
                {{-- FILTER LOKASI --}}
                <div>
                    <label class="block text-[10px] font-mono uppercase tracking-wider text-gray-500 mb-1">Lokasi</label>
                    <select name="city_code" class="w-full px-0 py-2 border-b-2 border-[#E5E5E5] focus:border-[#C1121F] outline-none bg-transparent text-[#111111] text-sm">
...
document.querySelectorAll('[class*="mini-calendar-"]').forEach(function(el) {
    ...
    if (card) {
        var nameEl = card.querySelector('.font-bold.text-\\[\\#111111\\]'), plateEl = card.querySelector('.text-xs.text-gray-500.font-mono');
