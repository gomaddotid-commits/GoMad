        ...
        <div x-show="searchMode === 'travel'">
            <form action="{{ route('search') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                <div class="col-span-1">
                    ...
                    <select name="origin" class="w-full px-0 py-2 border-b-2 border-[#E5E7EB] focus:border-[#BA1826] outline-none bg-transparent font-medium text-[#111827] appearance-none cursor-pointer transition-colors duration-300">
                ...
                <div class="col-span-1">
                    ...
                    <select name="destination" class="w-full px-0 py-2 border-b-2 border-[#E5E7EB] focus:border-[#BA1826] outline-none bg-transparent font-medium text-[#111827] appearance-none cursor-pointer transition-colors duration-300">
                        <option value="">Semua Kota</option>
                        @foreach($cities as $city)
                        <option value="{{ $city->name }}">{{ $city->name }}</option>
                        @endforeach
            ...
            <a href="{{ route('search', ['origin' => $route->origin_city_name, 'destination' => $route->destination_city_name]) }}" 
               class="card-gomad overflow-hidden flex-shrink-0 flex flex-row w-[calc(100%-1rem)] sm:w-[calc(50%-0.75rem)] lg:w-[calc(33.333%-1rem)] cursor-pointer p-0 h-32 md:h-40 group/card">
                ...
                <div class="w-2/3 p-4 flex flex-col justify-between">
                    <div>
                        <h3 class="font-bold text-[#111827] text-base md:text-lg truncate">{{ $route->route_name }}</h3>
                    ...
                    <div class="flex justify-between items-center border-t border-[#E5E7EB] pt-2 mt-2">
                        <p class="text-[10px] md:text-xs font-mono uppercase tracking-wider text-[#BA1826] font-medium">{{ $route->schedules_count ?? 0 }} jadwal</p>
