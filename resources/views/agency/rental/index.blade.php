...
<div>
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 border-b border-[#E5E7EB] pb-3">
        <h1 class="text-2xl font-bold text-[#111827]">Booking Rental</h1>
        <a href="{{ route('agency.rental.dashboard') }}" class="text-[#BA1826] text-sm hover:underline font-medium">
    ...
    <div class="flex gap-2 mb-6 overflow-x-auto pb-2">
        <a href="{{ route('agency.rental.index') }}" 
           class="px-4 py-2 rounded-full text-sm font-medium border whitespace-nowrap transition {{ !$statusFilter ? 'bg-[#BA1826] text-white border-[#BA1826]' : 'bg-white text-gray-600 border-[#E5E7EB] hover:border-[#BA1826]' }}">
            Semua
        </a>
        <a href="{{ route('agency.rental.index', ['status' => 'pending']) }}" 
           class="px-4 py-2 rounded-full text-sm font-medium border whitespace-nowrap transition {{ $statusFilter == 'pending' ? 'bg-yellow-500 text-white border-yellow-500' : 'bg-white text-gray-600 border-[#E5E7EB] hover:border-yellow-500' }}">
            ⏳ Pending
        </a>
        <a href="{{ route('agency.rental.index', ['status' => 'paid']) }}" 
           class="px-4 py-2 rounded-full text-sm font-medium border whitespace-nowrap transition {{ $statusFilter == 'paid' ? 'bg-blue-500 text-white border-blue-500' : 'bg-white text-gray-600 border-[#E5E7EB] hover:border-blue-500' }}">
            🚗 Siap Diambil
        </a>
        <a href="{{ route('agency.rental.index', ['status' => 'active']) }}" 
           class="px-4 py-2 rounded-full text-sm font-medium border whitespace-nowrap transition {{ $statusFilter == 'active' ? 'bg-indigo-500 text-white border-indigo-500' : 'bg-white text-gray-600 border-[#E5E7EB] hover:border-indigo-500' }}">
            🏃 Sedang Disewa
        </a>
        <a href="{{ route('agency.rental.index', ['status' => 'returned']) }}" 
           class="px-4 py-2 rounded-full text-sm font-medium border whitespace-nowrap transition {{ $statusFilter == 'returned' ? 'bg-orange-500 text-white border-orange-500' : 'bg-white text-gray-600 border-[#E5E7EB] hover:border-orange-500' }}">
            🔄 Dikembalikan
        </a>
        <a href="{{ route('agency.rental.index', ['status' => 'completed']) }}" 
           class="px-4 py-2 rounded-full text-sm font-medium border whitespace-nowrap transition {{ $statusFilter == 'completed' ? 'bg-green-500 text-white border-green-500' : 'bg-white text-gray-600 border-[#E5E7EB] hover:border-green-500' }}">
            ✅ Selesai
        </a>
    </div>
    ...
    <div class="bg-white border border-[#E5E7EB] rounded-[12px] p-12 text-center shadow-gomad">
        ...
        @if($statusFilter)
        <a href="{{ route('agency.rental.index') }}" class="text-[#BA1826] hover:underline mt-2 inline-block font-medium">Tampilkan semua</a>
        @endif
    </div>
    @else
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-[#E5E7EB]">
                    ...
                    <tr class="hover:bg-[#F9FAFB]">
                        ...
                        <td class="px-4 py-3">
                            <span class="font-medium text-[#111827] text-sm">{{ $rental->customer->name ?? '-' }}</span>
                            <br><span class="text-[10px] text-gray-500 font-light">{{ $rental->customer->phone ?? '-' }}</span>
                        </td>
                        <td class="px-4 py-3">
                            <span class="font-mono text-xs text-[#111827]">{{ $rental->vehicle->plate_number ?? '-' }}</span>
                        ...
                        <td class="px-4 py-3 text-right font-mono text-[#BA1826] font-medium">
                            Rp {{ number_format($rental->total_price, 0, ',', '.') }}
                        </td>
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('agency.rental.show', $rental) }}" class="text-[#BA1826] hover:underline text-xs font-medium">
                                Detail
                            </a>
                        </td>
                    </tr>
