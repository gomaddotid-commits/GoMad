...
Route::get('/seed1-' . env('SEED_TOKEN', 'default'), function () {
    ...
    $tables = ['wallet_transactions','withdrawals','notifications','passenger_transfer_bookings','passenger_transfers','promo_usages','referral_trackings','referral_codes','promo_schedule','promos','reviews','cash_payments','payments','booking_passengers','bookings','settlements','driver_locations','route_pricing','schedule_stops','schedules','user_devices','platform_settings','pickup_zones','agency_wallets','agency_verifications','vehicles','agencies','payment_agents','personal_access_tokens','failed_jobs','job_batches','jobs','cache_locks','cache','sessions','password_reset_tokens','users','route_stops','routes'];
    foreach ($tables as $t) { try { DB::table($t)->truncate(); $output .= "🧹 {$t}\n"; } catch (\Exception $e) {} }
    DB::statement('SET FOREIGN_KEY_CHECKS=1');
    
    $output .= "\n✅ All tables cleared!\n";
    
    $seeders = ['PlatformSettingSeeder','RouteSeeder','UserSeeder','PaymentAgentSeeder'];
    foreach ($seeders as $s) {
        try { Artisan::call('db:seed', ['--class' => $s, '--force' => true]); $output .= "✅ {$s}\n"; }
        catch (\Exception $e) { $output .= "❌ {$s}: " . $e->getMessage() . "\n"; }
    }
    
    $output .= "\n🎉 STEP 1 DONE! Lanjut ke /seed2-...";
    return response("<pre>{$output}</pre>");
});

// ONE-TIME SEEDER — Step 2: Schedules, Bookings, Payments, Promos
Route::get('/seed2-' . env('SEED_TOKEN', 'default'), function () {
    set_time_limit(0);
    $output = '';
    
    $seeders = ['AdditionalRouteSeeder','ScheduleSeeder','PromoSeeder','EnrichVerifiedDataSeeder'];
    foreach ($seeders as $s) {
        try { Artisan::call('db:seed', ['--class' => $s, '--force' => true]); $output .= "✅ {$s}\n"; }
        catch (\Exception $e) { $output .= "❌ {$s}: " . $e->getMessage() . "\n"; }
    }
    
    $output .= "\n🎉 STEP 2 DONE! Lanjut ke /seed3-...";
    return response("<pre>{$output}</pre>");
});

// ONE-TIME SEEDER — Step 3: Reviews, Notifications, Transfers, Wallet, Withdrawals + Auto-delete ALL
Route::get('/seed3-' . env('SEED_TOKEN', 'default'), function () {
    set_time_limit(0);
    $output = '';
    
    $seeders = ['WithdrawalSeeder','ReviewSeeder','NotificationSeeder','PassengerTransferSeeder','WalletTransactionSeeder'];
    foreach ($seeders as $s) {
        try { Artisan::call('db:seed', ['--class' => $s, '--force' => true]); $output .= "✅ {$s}\n"; }
        catch (\Exception $e) { $output .= "❌ {$s}: " . $e->getMessage() . "\n"; }
    }
    ...
    $content = preg_replace('/\/\/ ONE-TIME SEEDER.*?return response.*?\n\}\);\n/s', '// ONE-TIME SEEDER — DONE', $content);
